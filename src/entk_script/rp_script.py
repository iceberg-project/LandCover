...
'''
Radical Pilot Code for ICEBERG-LandCover
'''
import json
import argparse
...
if __name__ == '__main__':
    ...
    try:
        ...
        cud_pars.output_staging = {'source': 'unit:///landcover_images.json',
                                   'target': 'client:///landcover_images.json',
                                   'action': rp.TRANSFER}
        ...
        jsonfile = open("landcover_images.json", "r")
        ...
        cuds = list()
        report.header('submit LandCover Images units')
        ...
        for i in range(0, n):
            ...
            cud.pre_exec = [
                ...
                'LandCover/src:$PYTHONPATH',
                'export CUDA_VISIBLE_DEVICES=%d' % gpu_id]
            cud.executable = 'python'
            cud.arguments = ['%sLandCover/src/cal/' % args.input_pth +
                             'rad.py',
                             '--input_dir', 'Data']
            cud.input_staging = {
                'source': '%sLandCover/checkpoints_dir' % args.input_pth +
                          '/checkpoints_CVPR19W/' +
                          'v3weakly_unetr_bs96_main_model_ignore' +
                          '_bad/300_net_G.pth',
                'target': 'unit:///LandCover/checkpoints_dir' +
